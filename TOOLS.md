...
1. 收到 Steve 的審計請求（格式: "Patrick，我剛完成 XXX 合約開發，請幫忙審計..."）
2. 執行審計：
   - Slither 靜態分析
   - Foundry tests (forge test)
   - 手動 code review
3. `sessions_send` 結果回給 Steve
...
// Patrick 回報結果給 Steve
sessions_send({
  sessionKey: "agent:captain-hook:telegram:group:-1003723685993:topic:40",
  message: "Steve，審計完成。[結果摘要]",
});
```
